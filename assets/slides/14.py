...
class BaseAccount:
    def __init__(self, name, initial_deposit=0):
        self.name = name
        # Alternatively, we could have used self._balance to indicate that this is an internal variable.
    ...
    # We cannot have a function and an attribute with the same name
    # def balance(self):
...
BaseAccount.account_balance(my_account)
...
#data8.account_balance()
cs88.account_balance()
...
class BaseAccount2:
    ...
    # Note: BaseAccount2 uses _balance as a convention to indicate that this is internal
    # We use balance as a method to access the balance for clarity, but we could have just as easily used an attribute.
    # We could also define account_balance() if we wanted.
    def balance(self):
        return self._balance
